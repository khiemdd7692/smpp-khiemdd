...
trait TransmitterTrait
{
    ...
    public function submitSm($srcId, $mobile, $text)
    {
        ...
        if (GSMEncoder::isGsm0338($text)) {
            $dataEncoding = SMPPProtocol::DATA_CODING_DEFAULT;
...
//                }
                ...
                $esmClass = SMPPProtocol::ESM_CLASS_UDHI;
            } else {
//                $splitMessages = [Cmpp::packGsm7($text, 0)];
            ...
            $dataEncoding = SMPPProtocol::DATA_CODING_UCS2;
        ...
        if ($esmClass === SMPPProtocol::ESM_CLASS_UDHI) {
            $totalNum = count($splitMessages);

            $mark = $this->getLogSmsMark();

            foreach ($splitMessages as $index => $message) {
                $splitMessages[$index] = SMPPProtocol::packLongSmsSlice($message, $mark, $totalNum, $index + 1);
            }
        }
        ...
        foreach ($splitMessages as $msg) {
            ...
            $sequenceNum = SMPPProtocol::generateProSequenceId();

            $sequenceNums[] = $sequenceNum;

            $pdu = SMPPProtocol::packSubmitSm($srcId, $mobile, $msg, $sequenceNum, $esmClass, $dataEncoding);
